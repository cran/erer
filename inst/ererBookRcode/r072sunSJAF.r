...
wdNew <- "C:/aErer/aBookRcode"  # Set up working directory
setwd(wdNew); getwd()

# -------------------------------------------------------------------------
# 1. Import raw data in csv format
daInsNam <- read.table(file = "insData1.csv", header = TRUE, sep = ",")
daIns <- read.table(file = "insData2.csv", header = TRUE, sep = ",")
...
table.3 <- data.frame(daInsNam, Mean = insMean)[-14, ]
...
# -------------------------------------------------------------------------
# 4. Export results 
write.table(x = table.3, file = "insTable3.csv", sep = ",")
write.table(x = table.4, file = "insTable4.csv", sep = ",")