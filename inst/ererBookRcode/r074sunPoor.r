library(erer);wdNew<-"C:/aErer/aBookRcode";setwd(wdNew);getwd()
daInsNam=read.table("insData1.csv",header=TRUE,sep=",");daIns=read.table("insData2.csv",header=TRUE,sep=",");class(daInsNam);dim(daInsNam);print(daInsNam);class(daIns);dim(daIns);head(daIns);tail(daIns);daIns[1:3,1:5]
...
pname<-list(p1,p2,p3)
for(iin1:3){png(file=fname[i],width=4,height=3,
